...
- **protobuf (prior art)**: Protocol Buffers' wrapper types (`google.protobuf.StringValue`, `Int32Value`, etc.) address a similar problem — distinguishing "field absent" from "field present with value" in proto3, where scalar fields default to zero values. In Go, these generate `*wrapperspb.StringValue` (pointer to struct), providing two-state semantics (nil = absent, non-nil = present). However, protobuf wrappers are **two-state, not three-state**: JSON `null` for a wrapper field maps to the wrapper's default value (absent), not to an explicit null state. Protobuf solves the PATCH/partial-update problem via `google.protobuf.FieldMask` (an explicit list of fields to update) rather than encoding update intent into field types. This FieldMask approach works well for protobuf's RPC model but is not applicable to REST/JSON APIs where `null` conventionally means "clear this field" (e.g., JSON Merge Patch, RFC 7396). Our `Nullable[T]` extends the proven wrapper-type pattern to three states (`set` + `valid` flags) to cover the REST/JSON null semantic that protobuf's wrappers intentionally do not address.
